...
async def start_and_reset(dut):
    # Icarus uses 1 ps precision.  Pass an integer number of picoseconds so
    # both half-periods are represented exactly by cocotb.
    cocotb.start_soon(Clock(dut.clk, 64516, unit="ps").start())
    dut.ena.value = 1
    dut.ui_in.value = 0
    dut.uio_in.value = 0
    dut.rst_n.value = 0
